...
import dangerousSiteChecker, {
  DangerousSiteReport
} from "~/browserui/mixins/dangerous-site-checker";
...
export class Tab {
  ...
  public setUrl(url: string, ignoreDangerousSites: boolean = false) {
    ...
    if (url == "settings") {
      ...
      new DomainResolver(this._session.settings)
        .resolve(url)
        .then((response: any) => {
          if (response.type == "zil") {
            this.handleZilResponse(
              response.url,
              response.ipfsHash,
              response.dest,
              ignoreDangerousSites
            );
          } else {
            ipcRenderer.send(
              `load-new-url-${this.viewId}`,
              response.dest,
              response.url
            );
          }
  ...
  public buildBrowserView() {
    if (!this.viewId) {
      ...
      ipcRenderer.on(
        `view-title-updated-${this.viewId}`,
        (event, title: string) => {
          this._title = title;
        }
      );
  ...
  private handleZilResponse(
    url: string,
    ipfsHash: string,
    defaultDestination: string,
    ignoreDangerousSites: boolean
  ) {
    let dangerousSiteReport: DangerousSiteReport = dangerousSiteChecker.checkSite(
      url,
      ipfsHash
    );
    ...
    if (
      this._session.settings.ipfsContentMethod ==
